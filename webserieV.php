...
<style type="text/css">
  ...
  <header id="header" class="d-flex align-items-center">
    <div class="container d-flex justify-content-between align-items-center">
      ...
      <div class="logo">
        <h1><a href="webserieV.php">WebSerie</a></h1>
  ...
  <main id="main">
    ...
    <!-- ======= Featured Section ======= -->
    <?php
    $sel = $con->query("SELECT * FROM webserie");
    ?>
          ...
          <?php while ($row = $sel->fetch_assoc()) { ?>
            <div class="col-lg-4">
              <div class="card" style="width:  25rem;">
                <div class="icon-box">
                  <?php
                  if ($row['videowbs'] != NULL) {
                  ?>
                    <a style="padding-right: 240px" onclick="openModalV('<?php echo $row['videowbs'] ?>')"  type="button" title="Ver Video"><i class='bx bxs-videos'></i></a>
                  <?php
                  }
                  ?>
                  <?php
                  if ($row['imagen'] != NULL) {
                  ?>
                    <a onclick="openModalWbnI('<?php echo $row['imagen'] ?>')"  type="button" title="Ver Imagen"><i class='bx bxs-file-image'></i></a>
                  <?php
                  }
                  ?>
                  <center>
                    <h3><a><?php echo $row['titulo'] ?></a></h3>
                    <p  align="justify"><?php echo $row['contenidoWbs'] ?></p>
                    <br />
                    <p style="padding-left">Autor: <?php echo $row['autor_wbs'] ?> Fecha: <?php echo $row['fecha'] ?> Hora: <?php echo $row['hora'] ?></p>
                    <a href="mostrar.php?id_wbs=<?php echo $row['id_wbs'] ?>" class="card-link">Más Información</a>
                  </center>
                </div>
              </div>
            </div>
